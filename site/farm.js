...
function buildWorld(W, H, seed = 7, opts = {}) {
  const L = layoutWorld(W, H, opts), rnd = mulberry32(seed);
...
const UI = {
  ...
  // ---------------------------------------------------------------- textbox
  say(text) { if (!$("#textbox")) return; this.queue.push(text); if (!this.typing) this.next(); },
  next() {
