...
- Every seat must be **its owner's own login.** The farm never copies or shares credentials, and each person logs
  in on their own box with `claude-farm login`.
...
```bash
deploy/aws/deploy.sh up --workspace-repo git@github.com:your-org/your-repo.git   # stack "claude-farm", table "claude-farm"
...
```bash
STACK=farm-gil deploy/aws/deploy.sh up --table claude-farm --workspace-repo git@github.com:your-org/your-repo.git
...
SEAT matan-3f2a  boxes: claude-farm@a1c2, farm-2@c3d4
...
| Variable | Default | |
|---|---|---|
| `FARM_TABLE` | `claude-farm` | same value on every box of one farm |
